...
def train(model, NUM_EPOCHS, optimizer, DEVICE, train_loader, valid_loader, test_loader, args):
    ...
    # baseline 이어서 학습하는 경우
    if args.type == 'baseline':
        model_path = os.path.join('./', config.basemodel_path)
        if os.path.exists(model_path):
            print(f"Loading pre-trained baseline model from {model_path}")
    ...
    # margin 이어서 학습하는 경우
    if args.type == 'margin':
        model_path = os.path.join('./', config.margin_path)
        if os.path.exists(model_path):
            print(f"Loading pre-trained baseline model from {model_path}")
            model.load_state_dict(torch.load(model_path, map_location=DEVICE))
        else:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for _, (_, features, targets, z1, _) in enumerate(train_loader):
            ...
            if args.type == 'margin':
                ...
                logits, _, _, _, _ = model(features.float(), margins, s=config.scale)
        ...
        with torch.no_grad():
            ...
            if best_val < overall_acc:
                ...
                save_path = config.margin_path if args.type == 'margin' else config.basemodel_path
                save_state_dict(model.state_dict(), os.path.join('./', save_path))
